import { Purchase, Course, Chapter, Payment } from "@prisma/client";
import { EnrollmentPaymentDTO, PurchaseDTO, PaymentStatus } from "../purchaseDto";
...
// Definimos el tipo que junta Purchase con su Payment desde Prisma
type PurchaseWithPayment = Purchase & {
...
export function toEnrollmentPaymentDTO(
  ...
  return {
    purchase: !!purchase,
    payment: purchase?.payment ? (purchase.payment.status as PaymentStatus) : null,
  };
}

//api

export function toCourseWithPurchaseDTO(
  course: Course & { purchases: Purchase[] },
): CourseWithPurchaseDTO {
  return {
    ...toCourseDTO(course),
...
export function toCourseDetailsDTO(
  course: Course & { purchases: Purchase[]; chapters: Chapter[] },
): CourseDetailsDTO {
  return {
    ...toCourseDTO(course),
    purchases: toPurchasesDTO(course.purchases || []),
